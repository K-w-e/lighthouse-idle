...
export function createLand(scene: Phaser.Scene, lighthouse: Phaser.GameObjects.Sprite, landRadius: number): { landRT: Phaser.GameObjects.RenderTexture, landTiles: Phaser.Physics.Arcade.StaticGroup } {
...
export function createVision(scene: Phaser.Scene): { visionCone: Phaser.GameObjects.Graphics, visionMask: Phaser.Display.Masks.GeometryMask, waveness: Phaser.GameObjects.Graphics } {
    const visionCone = scene.add.graphics();
    const { width, height } = scene.scale;
    const waveness = scene.add.graphics()
...
export function drawLight(scene: Phaser.Scene, visionCone: Phaser.GameObjects.Graphics, lighthouse: Phaser.GameObjects.Sprite, angle: number) {
    visionCone.clear();
